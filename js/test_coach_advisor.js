...
// 场景 3: 清一色大番走向推荐 (手中条子达到9张，杂门牌优先舍弃)
{
  const game = G.createGame({ aiLevel: 'hard', round: 1, dealer: 0 });
  game.players[0].lack = 'm';
  // 条子 9张: s1,s2,s3, s4,s5,s6, s7,s8,s9, 筒子: p1, p9, p5, p5, p6 (共14张)
  game.players[0].hand = ['s1', 's2', 's3', 's4', 's5', 's6', 's7', 's8', 's9', 'p1', 'p9', 'p5', 'p5', 'p6'];
  game.turn = 0;
  game.phase = 'turn_act';

  const advice = MJAI.analyzeHandAdvice(game, 0);
  assert(advice.bestDiscard.startsWith('p'), `清一色规划推荐打出杂门筒子 (实际推荐: ${advice.bestDiscard})`);
